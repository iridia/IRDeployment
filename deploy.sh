...
FROM_DIR=`pwd`
TEMP_DIR="/tmp/Etoile.Da7VJ9" # `mktemp -d '/tmp/Etoile.XXXXXX'`	# Clean me up
PRODUCT_DIR="$TEMP_DIR/$BUILD_CONFIGURATION-$BUILD_SDK"

# clean

xcodebuild build -target "$TARGET_NAME" -configuration "$BUILD_CONFIGURATION" -sdk $BUILD_SDK CODE_SIGN_IDENTITY="$CODE_SIGN_IDENTITY" SYMROOT="$TEMP_DIR" PROVISIONING_PROFILE="$PROVISIONING_PROFILE"; bailIfError
